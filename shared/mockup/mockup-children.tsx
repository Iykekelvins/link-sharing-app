...
export default function MockupChildren({ links }: { links: LinkProps[] }) {
	const { links: linksStore, setLinks } = useLinkStore((s) => s);

	const user = useUserStore((s) => s.user);
	const clerkUser = useUser();

	// Sync the links from props to the store
	useEffect(() => {
		setLinks(links);
	}, [links, setLinks]);
		...
		<div
			...
			<div className='flex flex-col items-center relative z-2'>
				{!user ? (
					...
					<Image
						src={!user.image_url ? clerkUser.user?.imageUrl || '' : user?.image_url}
						width={96}
						height={96}
						alt={
							user.firstName
			...
			<ul className='flex flex-col w-full gap-5 px-9 relative z-2 mt-8'>
				{[...Array(5)].map((_, i) => {
					if (linksStore[i] && linksStore[i].platform) {
						const findIcon = PREVIEWLIST.find(
							(item) => item.platform === linksStore[i].platform,
						)?.icon;

						return (
							<li key={i}>
								<Link
									href={linksStore[i].url}
									prefetch={false}
									target='_blank'
									rel='noopener'
