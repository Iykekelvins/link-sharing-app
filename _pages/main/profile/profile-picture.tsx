...
export default function ProfilePicture({ form }: { form: UseFormReturn<FormData> }) {
	const fileInputRef = useRef<HTMLInputElement>(null);

	return (
		<div className='bg-light-grey mt-6 rounded-xl p-5'>
			<FormField
				control={form.control}
				name='profilePicture'
				render={({ field: { value, onChange, ...fieldProps } }) => (
					<FormItem className='grid items-center md:grid-cols-[0.5fr_1fr]'>
						<FormLabel className='text-grey text-base'>Profile picture</FormLabel>
						<FormControl>
							<div className='flex flex-col md:flex-row md:items-center gap-6'>
								<button
									...
									className={cn(
										...
										'h-48.5 min-w-48.5 w-max rounded-xl bg-light-purple mt-4 md:mt-0',
										'relative overflow-hidden hover:bg-light-purple/80 transition-colors',
									)}>
									{value ? (
										...
										<>
											...
											<p className='text-purple text-base font-semibold mt-2'>
												+ Upload Image
											</p>
										</>
									)}
